...
class room_parser(gsb.Parser):
	...
	def prompt(self, caller):
		caller.connection._("Enter ? to show all commands and room preferences")
	...
	def post_command(self, caller):
		...
		if caller.connection.parser is self:
			self.prompt(caller)
...
@RoomParser.command(names=['banlist'], args_regexp=r'([a-zA-Z0-9\.- ]+)', allowed = lambda c: not c.connection.player.room.open and c.collection.player.room.creator is c.connection.player)
def banlist(caller):
...
@RoomParser.command(names=['teams'], allowed c: c.connection.player.room.open)
def teams(caller):
	...
	for i in (1, 2):
		if len(room.teams[i]) == 0:
			pl.notify(pl._("No players in %s.")%(pl._("team %d")%(i+1)))
		else:
			pl.notify(pl._("Players in %s: %s")%(pl._("team %d")%(i+1), ', '.join([p.nickname for p in room.teams[i]]))

	if len(room.teams[0]) == 0:
		pl.notify(pl._("No remaining players in this room."))
	else:
		pl.notify(pl._("Players not yet in a team: %s")%(', '.join([p.nickname for p in self.teams[0]])))

@RoomParser.command(names=['move'], args_regexp=r'([0-2])', allowed c: c.connection.player.room.open)
def move(caller):
	...
	if len(caller.args) == 0 or len(caller.args) == 1 and caller.args[0] == None:
		...
		caller.args[0] = int(caller.args[0])
		room.move(pl, caller.args[0])
		if caller.args[0] == 0:
			...
			for p in room.get_all_players():
				if p is pl:
					pl.notify(pl._("You were moved into %s.")%(pl._("team %d")%(caller.args[0])))
				else:
					p.notify(p._("%s was moved into %s.")%(p._("team %d")%(caller.args[0])))
